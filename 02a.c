...
unsigned int
solve(char passwords[MAX_PASS][MAX_CHARS]) {
  size_t i, j;
  int min, max, char_count, valid_count = 0;
  char letter, password[MAX_CHARS];

  for (i=0; passwords[i][0] != '\0'; i++) {
    char_count = 0;

    sscanf(passwords[i], "%i-%i %c: %s", &min, &max, &letter, password);

    for (j=0; password[j] != '\0'; j++)
      if (password[j] == letter) char_count++;

    if (char_count >= min && char_count <= max) valid_count++;
  }
...
int
main() {
  char test_input[MAX_PASS][MAX_CHARS] = {
    "1-3 a: abcde",
    "1-3 b: cdefg",
    "2-9 c: ccccccccc",
    "",
